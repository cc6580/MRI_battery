...
Original file is located at
    https://colab.research.google.com/drive/1Jju7lpnSnVZtLY0tVnFRddPY5RtBN--4
"""
...
def calcfield(suscept, A):
  ...
  fieldz = magfield[fpl:2*fpl,0].reshape((rv.shape[0],rv.shape[1])) * 10e6
    # scale both by 10e6
    # now in unit of nano-tesla (10e3*10e6)
...
plt.savefig('0.025_training_33_input.png')
          ...
          for eps in epsilon_lst:
            ...
            for epo in epo_lst:
              for bat_size in bat_size_lst:
                ...
                if new_loss.numpy() < val_loss: # after each complete model training, assess
                  print()
...
for dist in set_dict.keys():
  for dp_rate in dp_list:
    ...
    best_params = grid_search(set_dict[dist]['train_images_t2b'], set_dict[dist]['train_labels_t2b'],
                              ...
                              dropout_level_lst = [dp_rate], epo_lst=[3000], bat_size_lst=[512])
