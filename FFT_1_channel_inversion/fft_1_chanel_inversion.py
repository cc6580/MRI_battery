# -*- coding: utf-8 -*-
"""FFT_1_chanel_inversion.py

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/10i8D89MpxdsuViWsGZ4xmHwOK3JUPmtK
"""
...
input_field=np.zeros((num_sim2, suscept_dims[2], suscept_dims[1],1), dtype=complex) 
...
assert np.all(input_field[30,:,:,0] == calcfield(output_suscept[30,:,:,:]))
...
plt.savefig('training_33_input_imag.png')

imshow_center(np.real(input_field[33,:,:,0]))
plt.savefig('training_33_input_real.png')

imshow_center(np.abs(input_field[33,:,:,0]))
plt.savefig('training_33_input_abs.png')
...
plt.savefig('training_33_label.png')
...
best_params = grid_search(dropout_level_lst=[0.01, 0.05], 
                          beta2_lst=[0.999], 
                          beta1_lst=[0.9], 
                          ...
                          bat_size_lst = [i*64 for i in range(3,0,-1)] # we always want to start big and get smaller because only the first best val_loss model gets saved, and we want that to be the most efficient if several parameters equally good loss eventually
                          )
print(100*'*')
...
assert id(best_pred) != id(worst_pred)